...
int cmd_ethernet(int argc, char **argv);
int cmd_fiber(int argc, char **argv);
...
int cmd_bios(int argc, char **argv) {
    (void)argc; (void)argv;
    printf("hep v9.0 — 92 commands — competing with bro (1 command)\n\n");
    printf("waves:\n");
    ...
    printf("  wave 8  — network:     ethernet  fiber  switch  packet\n");
    printf("                         ping  bandwidth  latency  bridge\n\n");
...
int main(int argc, char *argv[]) {
    ...
    if (!strcmp(cmd,"ethernet"))                return cmd_ethernet(argc-1, argv+1);
    if (!strcmp(cmd,"fiber"))                   return cmd_fiber(argc-1, argv+1);
    if (!strcmp(cmd,"switch"))                  return cmd_switch(argc-1, argv+1);
    if (!strcmp(cmd,"packet"))                  return cmd_packet(argc-1, argv+1);
    if (!strcmp(cmd,"ping"))                    return cmd_ping(argc-1, argv+1);
    if (!strcmp(cmd,"bandwidth"))               return cmd_bandwidth(argc-1, argv+1);
    if (!strcmp(cmd,"latency"))                 return cmd_latency(argc-1, argv+1);
    if (!strcmp(cmd,"bridge"))                  return cmd_bridge(argc-1, argv+1);
 ...
 * WAVE 8 — NETWORKING / COLLABORATION
 *
 * ethernet  — auto-generate changelog from wave messages
 * fiber     — show everything changed since a date
 * switch    — show what diverged between two branches
 * packet    — show one commit as a full detailed report
 * ping      — show all commits by a specific author
 * bandwidth — show which files change most frequently
 * latency   — show average time between commits
 * bridge    — export repo summary as shareable markdown
 * ════════════════════════════════════════════════════════════════════════════ */

/* ── ethernet: auto-generate changelog from commit messages ──────────────── */
int cmd_ethernet(int argc, char **argv) {
    (void)argc; (void)argv;
    char hex[41];
    if (repo_head_sha(hex) != HEP_OK || !hex[0]) {
        printf("ethernet: no commits yet\n"); return 0;
    }

    printf("# Changelog\n\n");
    printf("generated by hep ethernet\n\n");

    /* walk all commits, group by date (YYYY-MM) */
    char cur_month[8] = {0};
    int in_section = 0;
    ...
    while (hex[0]) {
        ...
        if (strcmp(month, cur_month) != 0) {
            if (in_section) printf("\n");
            char month_long[32];
            strftime(month_long, sizeof(month_long), "%B %Y", tm);
            printf("## %s\n\n", month_long);
            strncpy(cur_month, month, sizeof(cur_month)-1);
            in_section = 1;
        }
        ...
        if (msg[0] && strncmp(msg, "Merge", 5) != 0) {
            char sh[8]; memcpy(sh, hex, 7); sh[7] = '\0';
            printf("- %s (`%s`)\n", msg, sh);
        }
...
int cmd_fiber(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "fiber: Usage: hep fiber <date>\n");
        fprintf(stderr, "       e.g.   hep fiber 2025-01-01\n");
        fprintf(stderr, "              hep fiber \"7 days ago\"\n");
        return 1;
    }

    /* parse date — support YYYY-MM-DD or relative "N days/weeks ago" */
    time_t since = 0;
    int n; char unit[16];
    if (sscanf(argv[1], "%d %15s", &n, unit) == 2) {
        /* relative */
        time_t now = time(NULL);
        long secs = n;
        if (strncmp(unit, "day", 3) == 0)   secs *= 86400;
        else if (strncmp(unit, "week", 4) == 0) secs *= 7 * 86400;
        else if (strncmp(unit, "month", 5) == 0) secs *= 30 * 86400;
        since = now - secs;
    } else {
        /* YYYY-MM-DD */
        struct tm tm = {0};
        if (sscanf(argv[1], "%d-%d-%d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday) == 3) {
            tm.tm_year -= 1900; tm.tm_mon -= 1;
            since = mktime(&tm);
        } else {
            fprintf(stderr, "fiber: couldn't parse date '%s'\n", argv[1]);
            return 1;
        }
    }

    char since_str[32];
    strftime(since_str, sizeof(since_str), "%Y-%m-%d", localtime(&since));
    printf("fiber: changes since %s\n\n", since_str);
    ...
    while (hex[0]) {
        ...
        if ((time_t)c.commit_time >= since) {
            char sh[8]; memcpy(sh, hex, 7); sh[7] = '\0';
            char *msg = c.message ? c.message : "";
            char *nl = strchr(msg, '\n'); if (nl) *nl = '\0';
            char tstr[32];
            strftime(tstr, sizeof(tstr), "%Y-%m-%d", localtime((time_t*)&c.commit_time));
            printf("  %s  %s  %-20s  %s\n", sh, tstr,
                   c.author ? c.author : "unknown", msg);
            count++;
        }
    ...
    if (count == 0)
        printf("  nothing committed since %s\n", since_str);
    else
        printf("\n%d commit(s) since %s\n", count, since_str);
    return 0;
}
...
int cmd_switch(int argc, char **argv) {
    if (argc < 3) {
        fprintf(stderr, "switch: Usage: hep switch <branch1> <branch2>\n");
        return 1;
    }

    char root[HEP_PATH_MAX];
    if (repo_find_root(root, sizeof(root)) != HEP_OK) {
        fprintf(stderr, "switch: not in a hep repo\n"); return 1;
    }

    /* read tip of each branch */
    char ref1[HEP_PATH_MAX], ref2[HEP_PATH_MAX];
    snprintf(ref1, sizeof(ref1), "refs/heads/%s", argv[1]);
    snprintf(ref2, sizeof(ref2), "refs/heads/%s", argv[2]);

    char hex1[41] = {0}, hex2[41] = {0};
    repo_read_ref(ref1, hex1);
    repo_read_ref(ref2, hex2);

    if (!hex1[0]) { fprintf(stderr, "switch: branch '%s' not found\n", argv[1]); return 1; }
    if (!hex2[0]) { fprintf(stderr, "switch: branch '%s' not found\n", argv[2]); return 1; }

    /* collect commits reachable from each tip */
    /* simple approach: walk both, collect into sets, show unique to each */
    char **set1 = malloc(4096 * sizeof(char *)); int n1 = 0;
    char **set2 = malloc(4096 * sizeof(char *)); int n2 = 0;

    char h[41]; strncpy(h, hex1, 41);
    while (h[0] && n1 < 4096) {
        set1[n1++] = strdup(h);
        hep_commit c; if (commit_read(h, &c) != HEP_OK) break;
        if (c.parent_count > 0) sha1_to_hex(c.parents[0], h); else h[0]='\0';
        commit_free(&c);
    }
    strncpy(h, hex2, 41);
    while (h[0] && n2 < 4096) {
    ...
    printf("switch: divergence between '%s' and '%s'\n\n", argv[1], argv[2]);
    ...
    for (int i = 0; i < n1; i++) {
        int found = 0;
        for (int j = 0; j < n2; j++)
            if (strcmp(set1[i], set2[j]) == 0) { found = 1; break; }
        if (!found) {
            hep_commit c; char sh[8]; memcpy(sh,set1[i],7); sh[7]='\0';
            char *msg = "";
            if (commit_read(set1[i], &c) == HEP_OK) {
    ...
    for (int i = 0; i < n2; i++) {
        int found = 0;
        for (int j = 0; j < n1; j++)
            if (strcmp(set2[i], set1[j]) == 0) { found = 1; break; }
        if (!found) {
            hep_commit c; char sh[8]; memcpy(sh,set2[i],7); sh[7]='\0';
            if (commit_read(set2[i], &c) == HEP_OK) {
                char *msg = c.message ? c.message : "";
                char *nl = strchr(msg,'\n'); if(nl)*nl='\0';
                printf("  %s  %s\n", sh, msg);
                commit_free(&c);
    ...
    if (!only2) printf("  (none)\n");

    printf("\n%d unique to '%s', %d unique to '%s'\n",
           only1, argv[1], only2, argv[2]);

    for (int i=0;i<n1;i++) free(set1[i]);
    for (int i=0;i<n2;i++) free(set2[i]);
    free(set1); free(set2);
    return 0;
}

/* ── packet: show one commit as full detailed report ────────────────────── */
int cmd_packet(int argc, char **argv) {
    char hex[41];
    if (argc >= 2) {
        strncpy(hex, argv[1], 40); hex[40] = '\0';
    } else {
        if (repo_head_sha(hex) != HEP_OK || !hex[0]) {
            printf("packet: no commits yet\n"); return 0;
        }
    }

    hep_commit c;
    if (commit_read(hex, &c) != HEP_OK) {
        fprintf(stderr, "packet: couldn't read commit %s\n", hex); return 1;
    }

    char sh[8]; memcpy(sh, hex, 7); sh[7] = '\0';
    time_t t = (time_t)c.commit_time;
    char tstr[64]; strftime(tstr, sizeof(tstr), "%A %B %d %Y, %H:%M:%S", localtime(&t));
    ...
    printf("  date    : %s\n", tstr);
    if (c.parent_count > 0) {
        char ph[41]; sha1_to_hex(c.parents[0], ph);
        printf("  parent  : %s\n", ph);
    } else {
        printf("  parent  : (root commit)\n");
    }
    printf("  message :\n\n    %s\n\n", c.message ? c.message : "");
    ...
    hep_tree tree;
    if (tree_read(tree_hex, &tree) == HEP_OK) {
        printf("  files in this commit (%zu):\n", tree.count);
        for (size_t i = 0; i < tree.count; i++) {
            char bh[41]; sha1_to_hex(tree.entries[i].sha, bh);
            /* get blob size */
            hep_buf blob; long sz = 0;
            if (blob_read(bh, &blob) == HEP_OK) {
                sz = (long)blob.len; free(blob.data);
            }
            printf("    %-40s  %ld bytes  [%s]\n",
                   tree.entries[i].name, sz, bh);
        }
        tree_free(&tree);
    }
    printf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
...
int cmd_ping(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "ping: Usage: hep ping <author>\n");
        fprintf(stderr, "      e.g.   hep ping alice\n");
        return 1;
    }
    const char *target = argv[1];

    char hex[41]; repo_head_sha(hex);
    if (!hex[0]) { printf("ping: no commits yet\n"); return 0; }

    printf("ping: commits by '%s'\n\n", target);
    int count = 0;
    while (hex[0]) {
        hep_commit c;
        if (commit_read(hex, &c) != HEP_OK) break;

        if (c.author && strstr(c.author, target)) {
            char sh[8]; memcpy(sh, hex, 7); sh[7] = '\0';
            char *msg = c.message ? c.message : "";
            char *nl = strchr(msg, '\n'); if (nl) *nl = '\0';
            char tstr[32];
            strftime(tstr, sizeof(tstr), "%Y-%m-%d",
                     localtime((time_t*)&c.commit_time));
            printf("  %s  %s  %s\n", sh, tstr, msg);
            count++;
        }
...
int cmd_bandwidth(int argc, char **argv) {
    ...
    /* walk all commits, count how many times each file appears in a tree */
    typedef struct { char name[HEP_PATH_MAX]; int count; } FileCount;
    FileCount *files = calloc(4096, sizeof(FileCount));
    int nfiles = 0;
        ...
        if (tree_read(tree_hex, &tree) == HEP_OK) {
            for (size_t i = 0; i < tree.count; i++) {
                ...
                for (int j = 0; j < nfiles; j++) {
                    if (strcmp(files[j].name, tree.entries[i].name) == 0) {
                        files[j].count++;
                        found = 1; break;
                    }
                }
                if (!found && nfiles < 4096) {
                    strncpy(files[nfiles].name, tree.entries[i].name,
                            HEP_PATH_MAX-1);
    ...
    /* sort by count descending (simple bubble) */
    for (int i = 0; i < nfiles-1; i++)
        for (int j = 0; j < nfiles-1-i; j++)
            if (files[j].count < files[j+1].count) {
                FileCount tmp = files[j]; files[j] = files[j+1]; files[j+1] = tmp;
            }

    printf("bandwidth: most frequently changed files (%d commits total)\n\n",
           ncommits);
    int show = nfiles < 20 ? nfiles : 20;
    for (int i = 0; i < show; i++) {
        /* bar chart */
        int bar = (files[i].count * 30) / (files[0].count > 0 ? files[0].count : 1);
        char bar_str[32] = {0};
        for (int b = 0; b < bar && b < 31; b++) bar_str[b] = '█';
        printf("  %-35s  %3d  %s\n", files[i].name, files[i].count, bar_str);
    }
    if (nfiles == 0) printf("  no files tracked yet\n");
...
/* ── latency helpers ─────────────────────────────────────────────────────── */
static void fmt_duration(long s, char *buf, int len) {
    if (s < 60)        snprintf(buf, len, "%lds", s);
    else if (s < 3600) snprintf(buf, len, "%ldm %lds", s/60, s%60);
    else if (s < 86400)snprintf(buf, len, "%ldh %ldm", s/3600, (s%3600)/60);
    else               snprintf(buf, len, "%ldd %ldh", s/86400, (s%86400)/3600);
}
...
int cmd_latency(int argc, char **argv) {
    ...
    char hex[41]; repo_head_sha(hex);
    if (!hex[0]) { printf("latency: no commits yet\n"); return 0; }

    time_t *times = malloc(65536 * sizeof(time_t)); int n = 0;
    while (hex[0] && n < 65536) {
        hep_commit c;
        if (commit_read(hex, &c) != HEP_OK) break;
        times[n++] = (time_t)c.commit_time;
        if (c.parent_count > 0) sha1_to_hex(c.parents[0], hex);
        else hex[0] = '\0';
        commit_free(&c);
    }

    if (n < 2) { printf("latency: need at least 2 commits\n"); free(times); return 0; }

    /* compute gaps */
    long total = 0, min_gap = LONG_MAX, max_gap = 0;
    for (int i = 0; i < n-1; i++) {
    ...
    fmt_duration(avg, avg_str, sizeof(avg_str));
    fmt_duration(min_gap, min_str, sizeof(min_str));
    fmt_duration(max_gap, max_str, sizeof(max_str));

    char first[32], last[32];
    strftime(first, sizeof(first), "%Y-%m-%d", localtime(&times[n-1]));
    strftime(last,  sizeof(last),  "%Y-%m-%d", localtime(&times[0]));

    printf("latency: commit timing analysis\n\n");
    printf("  total commits   : %d\n", n);
    printf("  first commit    : %s\n", first);
    printf("  latest commit   : %s\n", last);
    printf("  avg between     : %s\n", avg_str);
    printf("  fastest gap     : %s\n", min_str);
    printf("  longest gap     : %s\n", max_str);

    /* rough activity by hour of day */
    int hours[24] = {0};
    for (int i = 0; i < n; i++) {
        struct tm *tm = localtime(&times[i]);
        hours[tm->tm_hour]++;
    }
    printf("\n  activity by hour:\n");
    int max_h = 0;
    ...
    for (int i = 0; i < 24; i++) {
        int bar = max_h > 0 ? (hours[i] * 20) / max_h : 0;
        char bar_str[22] = {0};
        for (int b=0;b<bar&&b<21;b++) bar_str[b]='▪';
        printf("  %02d:00  %s %d\n", i, bar_str, hours[i]);
    }
    free(times);
    return 0;
...
int cmd_bridge(int argc, char **argv) {
    const char *out_file = argc >= 2 ? argv[1] : NULL;
    FILE *out = out_file ? fopen(out_file, "w") : stdout;
    if (!out) { fprintf(stderr, "bridge: couldn't open '%s'\n", out_file); return 1; }
    ...
    /* count commits */
    int ncommits = 0;
    char h[41]; strncpy(h, hex, 41);
    char first_msg[256] = {0}; time_t first_t = 0, last_t = 0;
    char first_author[256] = {0};
    while (h[0]) {
        hep_commit c; if (commit_read(h, &c) != HEP_OK) break;
        ncommits++;
        if (last_t == 0) last_t = (time_t)c.commit_time;
        first_t = (time_t)c.commit_time;
        if (c.message) strncpy(first_msg, c.message, 255);
        if (c.author)  strncpy(first_author, c.author, 255);
        char *nl = strchr(first_msg,'\n'); if(nl)*nl='\0';
        if (c.parent_count > 0) sha1_to_hex(c.parents[0], h); else h[0]='\0';
        commit_free(&c);
    }

    char first_date[32], last_date[32];
    strftime(first_date, sizeof(first_date), "%Y-%m-%d", localtime(&first_t));
    strftime(last_date,  sizeof(last_date),  "%Y-%m-%d", localtime(&last_t));

    /* get file list from HEAD tree */
    hep_tree tree; int has_tree = 0;
    if (hex[0]) {
        hep_commit c;
        if (commit_read(hex, &c) == HEP_OK) {
            char th[41]; sha1_to_hex(c.tree_sha, th);
            if (tree_read(th, &tree) == HEP_OK) has_tree = 1;
            commit_free(&c);
        }
    }

    /* write markdown */
    char sh[8]; if (hex[0]) { memcpy(sh, hex, 7); sh[7]='\0'; } else strcpy(sh, "none");
    time_t now = time(NULL);
    char now_str[32]; strftime(now_str, sizeof(now_str), "%Y-%m-%d %H:%M", localtime(&now));

    fprintf(out, "# hep repo summary\n\n");
    fprintf(out, "_generated %s_\n\n", now_str);
    fprintf(out, "---\n\n");
    fprintf(out, "## overview\n\n");
    fprintf(out, "| | |\n|---|---|\n");
    fprintf(out, "| branch | `%s` |\n", branch);
    fprintf(out, "| HEAD | `%s` |\n", sh);
    fprintf(out, "| commits | %d |\n", ncommits);
    fprintf(out, "| started | %s |\n", first_date);
    fprintf(out, "| latest | %s |\n", last_date);
    if (first_author[0])
        fprintf(out, "| first author | %s |\n", first_author);
    fprintf(out, "\n");

    if (has_tree && tree.count > 0) {
    ...
    /* recent commits */
    fprintf(out, "## recent commits\n\n");
    strncpy(h, hex, 41);
    int shown = 0;
    while (h[0] && shown < 10) {
        hep_commit c; if (commit_read(h, &c) != HEP_OK) break;
        char csh[8]; memcpy(csh, h, 7); csh[7]='\0';
        char *msg = c.message ? c.message : "";
        char *nl = strchr(msg,'\n'); if(nl)*nl='\0';
        char tstr[16]; strftime(tstr,sizeof(tstr),"%Y-%m-%d",
                                localtime((time_t*)&c.commit_time));
        fprintf(out, "- `%s` %s — %s _%s_\n",
                csh, tstr, msg, c.author ? c.author : "unknown");
        shown++;
        if (c.parent_count > 0) sha1_to_hex(c.parents[0], h); else h[0]='\0';
        commit_free(&c);
    }
    ...
    if (out_file) {
        fclose(out);
        printf("bridge: wrote report to '%s'\n", out_file);
    }
    return 0;
}
